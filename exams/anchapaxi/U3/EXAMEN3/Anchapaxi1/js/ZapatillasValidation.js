...
function validatePhone() {
  ...
  let regex = /^[0-9]{10}$/;
  let lastIdError = document.getElementById('phone-error');

  if (id.trim() === '') {
      lastIdError.innerText = "Please enter the phone.";
      lastIdError.style.display = 'block';
      lastIdError.style.color = 'red';
      return false;
    } else if (!regex.test(lastname)) {
      lastIdError.innerText = "!Warning: Invalid phone";
      lastIdError.style.display = 'block';
      lastIdError.style.color = 'red';
      return false;
    }else {
...
function validateDate(event) {
    ...
    if (fechaIngresada > fechaActual) {
        dateError.innerText = "!Warning: The date entered is incorrect.";
